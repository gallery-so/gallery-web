...
export const fetchSanityContent =
  (sanityProjectId: string | undefined) => async (query: string) => {
    ...
    const url =
      typeof window !== 'undefined' && sanityProjectId === 'i8s3oao1'
        ? `/__sanity/query?query=${encodedQuery}`
        : `https://${sanityProjectId}.api.sanity.io/v1/data/query/production?query=${encodedQuery}`;
